# Introduction #
Live demo:<br />
[![](http://word-cumulus-goog-vis.googlecode.com/svn/trunk/icon-ballcloud.png)](http://word-cumulus-goog-vis.googlecode.com/svn/trunk/demo.html)

This visualization show a rotating ball of tags, each linking to some url (on a user click). You're able to control speed, color, and the font size for each tag individually.
The widget itself uses flash, and it is an open-source. See http://wordpress.org/extend/plugins/wp-cumulus/ and http://www.roytanck.com/2008/05/19/how-to-repurpose-my-tag-cloud-flash-movie/

# Details #

## Setup ##
  * Include the js files http://word-cumulus-goog-vis.googlecode.com/svn/trunk/swfobject.js and http://word-cumulus-goog-vis.googlecode.com/svn/trunk/wordcumulus.js in your html header
  * Create a div container for the visualization

## Inputs ##
The DataTable format accepted by the visualization, assumes the first column (string) has the user-text to show, and the second column (string) has the target urls. An optional third column (number) can contain font sizes.<br />
Options properties that may be passed to draw:
  * text\_color (e.g., '#ff0000')
  * hover\_text\_color
